...
#-----------------------------------------------------------------------------------------------------
async def fetch(data_inicio, data_fim, orgao):
    if not data_inicio or not data_fim or not orgao:
        return JSONResponse(
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        try:
            if orgao == '':
                url = f'https://portaltransparencia-back.sepog.fortaleza.ce.gov.br/api/despesas/detalhadas/diarias/{data_inicio}/{data_fim}/empenho'
            else:
                url = f'https://portaltransparencia-back.sepog.fortaleza.ce.gov.br/api/despesas/detalhadas/diarias/{data_inicio}/{data_fim}/empenho?orgao={orgao}'
            ...
            async with session.get(url, ssl=False, allow_redirects=True) as resp:
                ...
                if not response_data:
                    result = ResponseDefault(
                        code=0,
                        message='Não foi encontrado despesas para o período informado',
                        results=[],
